...
class HeadHunterDataBase(LoggingConfigClassMixin):
    ...
    def _execute(self, query, params=None, log_info: str=""):
        with self.conn.cursor() as cur:
            try:
                cur.execute(query, params)
                return cur.fetchall()
            except psycopg2.Error as e:
                self.logger.error(f"Ошибка при работе базой данных: {e}")
            else:
    ...
    def create_database(self) -> None:
        ...
        is_exists = self._execute("SELECT 1 FROM pg_database WHERE datname = %s", (self._hh_dbname,))
        if is_exists:
            self._execute(f"DROP DATABASE {self._hh_dbname}")
            self.logger.info(f"База данных {self._hh_dbname} удалена")
        self._execute(f"CREATE DATABASE {self._hh_dbname}")
        self.logger.info(f"База данных {self._hh_dbname} создана")
